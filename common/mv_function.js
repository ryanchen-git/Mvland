...
function show_favorite() {
	if(action_open == 0) {
		...
		x.innerHTML = '<br /><div style="border: 1px solid #6A259C; width: 85%;"><form action="http://www.mvland.com/watch/mv=unique_id" method="post" id="message_form_fly"><table border="0" width="100%"><tr><td><strong>確定存此MV到我的最愛?</strong></td></tr><tr><td height="5"></td></tr><tr><td><input type="submit" value="確定" style="width: 100px; height: 30px;" /><input type="button" value="取消" onclick="hide_favorite()" style="width: 100px; height: 30px;" /><td></tr></table><input type="hidden" name="add_favorites" /></form></div>';
